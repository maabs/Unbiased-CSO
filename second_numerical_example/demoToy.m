...
load data_daily.mat;
load eurostoxx50_daily_adj_close_clean.mat;
...
adj_q(1:5,2)
size(data_q);
size(find( abs(exp(data_q(:,2:end)).*adj_q(:,1:end-1)-adj_q(:,2:end))<100000))
size(data_q,1)*(size(data_q,2)-1)- (nnz(isnan(data_q)))

size(find( abs(exp(data_q(:,2)).*adj_q(:,1)-adj_q(:,2))<0.01))
time=502;
abs(exp(data_q(:,time+1 )).*adj_q(:,time)-adj_q(:,time+1))


%%
model.horizon  = 100; %forecast horizon
...
mcmcoptions.train.StoreEvery = 2;
aOpts = struct();
saOpts.Ksa       = 200;      % number of SA iterations
saOpts.gamma     = 1;       % risk aversion
saOpts.blockSize = 512;     % for Sigmar_times_beta_exact / matvec batching
saOpts.a0        = 400000;     % base stepsize
saOpts.aPow      = 0.6;     % stepsize exponent
saOpts.xiClip    = 20;      % stabilize softmax
saOpts.seed      = randi(100000);   % reproducibility
saOpts.reusePropDist = true;
saOpts.threadCount   = 1;   % set [] to not force
saOpts.verbose       = true;
...
plot(out.obj_proxy_hist); grid on; title('SA objective proxy');
%%
find(out.beta_hist(:,end)>0.05)
out.beta_hist(find(out.beta_hist(:,end)>0.05),end)
out.beta_hist(:,end-8:end)
exp(sum(data_q(find(out.beta_hist(:,end)>0.1),end-model.horizon+1:end),2))
%%
...
fname = sprintf('repro_online_%s.mat', datestr(now,'yyyymmdd_HHMMSS'));
