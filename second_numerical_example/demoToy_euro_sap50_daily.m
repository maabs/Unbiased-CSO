...
%%
% Here I check that the log returns correspond to the adjusted close
% prices.
adj_d(1:5,2)
size(data_d);
size(find( abs(exp(data_d(:,2:end)).*adj_d(:,1:end-1)-adj_d(:,2:end))<100000))
size(data_d,1)*(size(data_d,2)-1)- (nnz(isnan(data_d)))

size(find( abs(exp(data_d(:,2)).*adj_d(:,1)-adj_d(:,2))<0.01))
time=502;
abs(exp(data_d(:,time+1 )).*adj_d(:,time)-adj_d(:,time+1))
...
aOpts = struct();
saOpts.Ksa       = 210;      % number of SA iterations
saOpts.gamma     = 10;       % risk aversion
saOpts.blockSize = 512;     % for Sigmar_times_beta_exact / matvec batching
saOpts.a0        = 100000;     % base stepsize
saOpts.aPow      = 0.6;     % stepsize exponent
saOpts.xiClip    = 20;      % stabilize softmax
...
plot(out.obj_proxy_hist); grid on; title('SA objective proxy');
%%
find(out.beta_hist(:,end)>0.01)
out.beta_hist(find(out.beta_hist(:,end)>0.01),end)
out.beta_hist(:,end);
%%
mcmcoptions.train.T = 50;
mcmcoptions.train.Burnin = 0;
...
onlineOpts.saInnerIters = 2;       % SA updates per day
...
%%
[row,column]=find(outOnline.beta>0.05);
[row,column]
outOnline.beta(find(outOnline.beta>0.05));
...
fname = sprintf('repro_online_sp_252d_every1_%s.mat', datestr(now,'yyyymmdd_HHMMSS'));
