...
aOpts = struct();
saOpts.Ksa       = 40;      % number of SA iterations
saOpts.gamma     = 1;       % risk aversion
saOpts.blockSize = 512;     % for Sigmar_times_beta_exact / matvec batching
saOpts.a0        = 8000;     % base stepsize
saOpts.aPow      = 0.6;     % stepsize exponent
saOpts.xiClip    = 20;      % stabilize softmax
...
plot(out.obj_proxy_hist); grid on; title('SA objective proxy');
%%
find(out.beta_hist(end-model.horizon:end,end)>0.1);
out.beta_hist(find(out.beta_hist(end-model.horizon:end,end)>0.1),end);
out.beta_hist(:,end-model.horizon:end)
