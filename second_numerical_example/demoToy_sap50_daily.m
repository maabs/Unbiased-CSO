...
load  stoxx600_daily_adj_close_and_rets_last.mat;
...
time_d(end)
%figure;
%plot(adj_d(100,end-1000:end))
%%
data_d=data_d(1:300,end-756:end);
time_d=time_d(:,end-756:end);
adj_d=adj_d(1:300,end-756:end);
size(data_d)
...
%%
sanityCheckMSV('postMCMC', model, samples, mcmcoptions);
sanityCheckMSV('preForecast', model, samples, mcmcoptions);

fc_per = msv_forecast_demoToy(samples, mcmcoptions, model, 'staticmean');
%%
fc_only= msv_forecast_states_only(samples, model, 'staticmean');
size(fc_per.Ferror_rel)
figure;
semilogy(abs( fc_per.Ferror_rel))
% Variant A: static params 
% fixed to posterior mean (Scalable 6.2-style)
%fc_mean = msv_forecast_demoToy(samples, mcmcoptions, model, 'staticMean');
...
% Y           : N x Ttrain
% model.actual: N x H
% forecast_output.LogScoreMix (optional): H x 1

H = model.horizon;
Yhold = model.actual;   % N x H

% ---- Baseline A: constant covariance ----
Sigma_const = cov_pairwise_nan(Y);

LL_const = NaN(H,1);

for h=1:H
    LL_const(h) = gaussLogScore(Yhold(:,h), Sigma_const);
end
% ---- Baseline B: EWMA covariance ----
alpha = 0.06; % common RiskMetrics-ish choice; you can tune this
Sigmas_ewma = ewma_cov_nan(Y, alpha, H);

LL_ewma = NaN(H,1);
for h=1:H
    LL_ewma(h) = gaussLogScore(Yhold(:,h), Sigmas_ewma(:,:,h));
end

% ---- Your MSV score ----
% If you computed mixture scores:
LL_msv = fc_per.LogScoreMix(:);

% ---- Summaries ----
avg_const = mean(LL_const,'omitnan');
avg_ewma  = mean(LL_ewma,'omitnan');
avg_msv   = mean(LL_msv,'omitnan');

fprintf('Average log score (higher is better):\n');
fprintf('  ConstCov : %.6f\n', avg_const);
fprintf('  EWMA     : %.6f\n', avg_ewma);
fprintf('  MSV      : %.6f\n', avg_msv);

% Differences (interpretable as log predictive density ratios)
d_msv_const = avg_msv - avg_const;
d_msv_ewma  = avg_msv - avg_ewma;

fprintf('\nAverage improvement (MSV - baseline):\n');
fprintf('  vs ConstCov: Δ=%.6f  => density x %.4f\n', d_msv_const, exp(d_msv_const));
fprintf('  vs EWMA    : Δ=%.6f  => density x %.4f\n', d_msv_ewma,  exp(d_msv_ewma));
%%


beta = ones(model.N,1)/model.N;      % example beta (simplex)
blockSize = 512;                      % tune as needed
...
Sigmar_beta = zeros(model.N, H, S);
mu_r = zeros(model.N, H, S);

for s = 1:S
    for hh = 1:H
        omegas  = fc_only.omegas_fc(:, hh, s);    % R x 1
        lambdas = fc_only.lambdas_fc(:, hh, s);   % K x 1

        [Sigmar_beta(:, hh, s) ,mu_r(:, hh, s)]= Sigmar_times_beta_exact( ...
            beta, LW, omegas, lambdas, sig2, model.Givset, blockSize);
    end
end

%%
beta = ones(model.N,1)/model.N;
M = 200;  % MC inner samples per (h,s)

LW   = fc_only.static.LW_mean;
sig2 = fc_only.static.sigma2_mean;

H = model.horizon;
S = size(fc_only.lambdas_fc,3);

SigmarBetaHat = zeros(model.N,H,S);
...
for s=1:S
  for hh=1:H
    ...
    SigmarBetaHat(:,hh,s) = mc_estimate_Sigmar_beta( ...
        LW, lambdas, omegas, sig2, model.Givset, beta, M);
  end
end
...
g_exact = Sigmar_times_beta_exact(beta, LW, omegas, lambdas, sig2, model.Givset, blockSize);

M = 20000000;
g_mc = mc_estimate_Sigmar_beta(LW, lambdas, omegas, sig2, model.Givset, beta, M);
rel2   = norm(g_mc - g_exact) / norm(g_exact);
relinf = norm(g_mc - g_exact, inf) / norm(g_exact, inf);
coserr = 1 - (g_mc' * g_exact) / (norm(g_mc)*norm(g_exact));
size(g_mc)
%%
...
beta  = ones(model.N,1)/model.N;
gamma = 1;
blockSize = 512;   % set to whatever you used in Sigmar_times_beta_exact

[grad_mean,grad_xi_mean, grad_hs, mu_mean, SigmarBeta_mean] = grad_msv_exact( ...
    beta, fc_only, model, gamma, blockSize);
%%

% --- Example call for SA (maximize beta' mu_r - (gamma/2) beta' Sigma^r beta) ---

% Ensure model.N, model.horizon, model.Givset, etc. are already set,
% and mcmcoptions.adapt / mcmcoptions.train exist.
...
mcmcoptions.train.T = 100;
mcmcoptions.train.Burnin = 0;
mcmcoptions.train.StoreEvery = 2;


saOpts = struct();
saOpts.Ksa       = 50;      % number of SA iterations
saOpts.gamma     = 1;       % risk aversion
saOpts.blockSize = 512;     % for Sigmar_times_beta_exact / matvec batching
saOpts.a0        = 50000;     % base stepsize
saOpts.aPow      = 0.6;     % stepsize exponent
saOpts.xiClip    = 20;      % stabilize softmax
saOpts.seed      = 12345;   % reproducibility
...
[out,adaptBudle] = sa_portfolio_msv(model, mcmcoptions, Langevin, saOpts);
...
plot(out.obj_proxy_hist); grid on; title('SA objective proxy');
...
%%
model.horizon=10;
mcmcoptions.train.T = 50;
mcmcoptions.train.Burnin = 0;
mcmcoptions.train.StoreEvery = 1;
aOpts = struct();
saOpts.Ksa       = 50;      % number of SA iterations
saOpts.gamma     = 10;       % risk aversion
saOpts.blockSize = 512;     % for Sigmar_times_beta_exact / matvec batching
saOpts.a0        = 1000*30/saOpts.gamma;     % base stepsize
saOpts.aPow      = 0.6;     % stepsize exponent
saOpts.xiClip    = 20;      % stabilize softmax
...
plot(out.obj_proxy_hist); grid on; title('SA objective proxy');
 ...
 %%
find(out.beta_hist(:,end)>0.02)
out.beta_hist(find(out.beta_hist(:,end)>0.02),end)
out.beta_hist(:,end);
%%
figure;
...
onlineOpts = struct();
model.horizon=252;
onlineOpts.T0           = size(data_d,2)-model.horizon;    % initial training length
onlineOpts.H            = model.horizon;      % number of online days to run
onlineOpts.saInnerIters = 5;       % SA updates per day
...
saOptsBase.a0        = 3000*30/saOptsBase.gamma;     % base stepsize
saOptsBase.aPow      = 0.6;
...
[row,column];
outOnline.beta(find(outOnline.beta>0.01));
ph=(outOnline.beta>0.01);
outOnline.beta(end-100:end)
%%
t = 50;                    % pick time
w = outOnline.beta(:, t);   % N x 1
N = numel(w);
i = 1:N;
figure;
stem(i, w, 'filled'); hold on; grid on;
...
ylabel('\beta_{i,t}');
title(sprintf('Weights at time t = %d', t));
legend('\beta_{i,t}', 'Uniform weight 1/N',00, 'Location','best');
%% ===============================
% Reproducibility pack: save state
% ===============================
repro = struct();
% ---- Core output000s you want to keep ----
...
repro.model       = model;         % includes model.Y/data_d as you set it
...
repro.data_d      = data_d;        % N x T log-returns
if exist('time_d','var')
    repro.time_d  = time_d;        % 1 x T (strings or datenums)
end

% ---- If you have prices too, store them (optional but helpful) ----
if exist('adj_close','var')
    repro.adj_close = adj_close;   % N x T prices aligned with data_d
end
...
fname = sprintf('repro_online_tadawul_252d_every1_%s_gamma12_top_239_latest_dates.mat', datestr(now,'yyyymmdd_HHMMSS'));
%save(fullfile(out_dir, fname), 'repro', '-v7.3');

fprintf('\nSaved reproducibility pack:\n  %s\n', fullfile(out_dir, fname));
%%
% Path to the file you saved
%out_dir = fullfile(pwd, 'repro_runs');
%file = fullfile(out_dir, 'repro_online_sp_252d_every1_20260311_092521_gamma12_top_300.mat');
%S = load(file, 'repro');   % load ONLY variable 'repro'
%repro = S.repro;           % bring it into workspace
% % Now access fields
%outOnline   = repro.outOnline;
...
Y_trade = data_d(:, T0+1 : T0+H);   % [N x H] log-returns (may contain NaN)
...
% Requires: data_d (N×T log-returns), outOnline.beta (N×H), model.horizon = H
...
% ===============================
[N, T] = size(data_d(:,end-model.horizon+1:end));

% Convert log returns to simple returns
R = exp(data_d(:,end-model.horizon+1:end)) - 1;   % [N x T]
