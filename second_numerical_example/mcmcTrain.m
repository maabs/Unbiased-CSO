...
samples.F = zeros(num_stored, n); % Latent hs and deltas
samples.Ft = zeros(model.K, model.T, num_stored); % latent f_t (only makes sense
%  in the full factor model, f_t ~ N(0,\Sigma_t) 
samples.Phi_h = zeros(num_stored, model.K); % Persistent parameters
samples.Phi_delta = zeros(num_stored, model.tildeK); % Persistent parameters
samples.h_0 = zeros(num_stored, model.K);
samples.delta_0 = zeros(num_stored, model.tildeK);
samples.sigma2_h = zeros(num_stored, model.K); % error variances of the AR
% processes
samples.sigma2_delta = zeros(num_stored, model.tildeK);
samples.sigma2 = zeros(1, num_stored); % variance of the full factor additional
% term (V_t^{1/2} \epsilon_t)
samples.Weights = zeros(model.N, model.K, num_stored); % Weights of the full 
% factor model. B in Bf_t.
samples.LogL = zeros(1, num_stored); % stores log likelihoods.
cnt = 0;
acceptF = 0;
...
% tridiagonal matrix
% i.e., the precision matrix, which in case of a Gaussian first order
% markovian process is a tridiagonal (encodes the dependances)
%W = spalloc(n,n,3*n-2);
iAll = []; 
jAll = [];
en = 0; 
...
% we start allocating inputs from model. 
...
if idx - 1 ~= nnz_total
    error('nonZeros fill mismatch: idx-1 = %d, nnz_total = %d', idx-1, nnz_total);
end
...
%
        ...
        for t=1:model.T 
            ...
            if length(ind) > 0
               ...
               for k=size(model.Givset,1):-1:1
                   %c = cos(model.omegas(k,t));
                   %s = sin(model.omegas(k,t));    
                   %tmp = v; This operation is O(K), we´ve replaced it with so that it's O(1).
   ...
   %    
    ...
    % STEP 4: Update h_0s and delta_0s

    

    % local switches
    % This option is implemented in order to 
    % make the the variables h_0 and delta_0 
    % random as opposed to the original implementation (error?)
    sample_h0_randomly = 'no';
    sample_delta0_randomly = 'no';
    
    % ---------- h_0 update ----------
    t1 = 1 - model.phi_h; 
    t2 = 1 - model.phi_h.^2;
    
    h0_num = t2 .* (model.hs(:,1)') ...
        + t1 .* (sum(model.hs(:,2:end) ...
        - repmat(model.phi_h', 1, model.T-1) .* model.hs(:,1:end-1), 2)');
    
    h0_den = t2 + (model.T-1) * (t1.^2);
    
    if strcmp(sample_h0_randomly, 'yes')
        sh2 = model.sigma2_h ./ h0_den;
        model.h_0 = h0_num ./ h0_den + sqrt(sh2) .* randn(size(sh2));
    else
        model.h_0 = h0_num ./ h0_den;
    end
    ...
    delta0_num = t2 .* (model.deltas(:,1)') ...
        + t1 .* (sum(model.deltas(:,2:end) ...
        - repmat(model.phi_delta', 1, model.T-1) .* model.deltas(:,1:end-1), 2)');
    
    delta0_den = t2 + (model.T-1) * (t1.^2);
    
    if strcmp(sample_delta0_randomly, 'yes')
        sdelta2 = model.sigma2_delta ./ delta0_den;
        model.delta_0 = delta0_num ./ delta0_den + sqrt(sdelta2) .* randn(size(sdelta2));
    else
        model.delta_0 = delta0_num ./ delta0_den;
    end
    ...
    nonZeros = zeros(1, nnz_total);   % preallocate
    idx = 1;                          % pointer
    for i=1:model.K
    ...
    % STEP 6:  SAMPLE ALL PHIS (M-H step)
        % ---------------------------------------------------------
    % IMPORTANT: refresh oldLogLphi after updating:
    %   h_0, delta_0, sigma2_h, sigma2_delta, W, Lfree, mu
    % so that the phi-MH ratio is computed at the CURRENT state.
    % ---------------------------------------------------------
    ok = Lfree * (F - mu);
    oldLogLphi = - 0.5*(model.T*(model.K + model.tildeK))*log(2*pi) ...
                 + sum(log(diag(Lfree))) ...
